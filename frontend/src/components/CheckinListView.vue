	...
	<ion-content>
		...
		>
			<div class="w-full">
				...
				>
					<template v-if="isGroupedData">
						<div v-for="(group, groupKey) in groupedDocuments" :key="groupKey" class="mb-4">
							<!-- Date Header -->
							<div class="p-3 bg-gray-100 font-medium text-gray-800">
...
const documents = createResource({
	...
	transform(data) {
		...
		console.log('Transformed data:', pagedData)
		return pagedData
	},
})
...
function fetchDocumentList(start = 0) {
	...
	// Ensure we have the time field for grouping by date
	if (props.groupBy === 'DATE(time)' && !listOptions.value.fields.includes('time')) {
		listOptions.value.fields.push('time')
	}
...
// Add these computed properties and methods after the existing computed properties
const isGroupedData = computed(() => {
	return (props.groupBy || props.clientSideGroupBy) && documents.data?.length > 0
})

const groupedDocuments = computed(() => {
	if (!isGroupedData.value) return {}
	
	const grouped = {}
	documents.data.forEach(doc => {
		// For DATE(time) grouping, extract the date part
		let groupKey = ''
		if (props.clientSideGroupBy && doc[props.clientSideGroupBy]) {
			// For client-side grouping, extract the date part
			groupKey = dayjs(doc[props.clientSideGroupBy]).format('YYYY-MM-DD')
		} else if (props.groupBy === 'DATE(time)' && doc.time) {
			groupKey = dayjs(doc.time).format('YYYY-MM-DD')
		} else {
			// For other groupBy fields, use the field value directly
			const fieldName = props.groupBy ? props.groupBy.replace(/^.*\(([^)]+)\).*$/, '$1') : ''
			groupKey = doc[fieldName] || 'Unknown'
		}
...
function formatGroupHeader(groupKey) {
	if (props.groupBy === 'DATE(time)' || props.clientSideGroupBy) {
		return dayjs(groupKey).format('dddd, D MMMM YYYY')
	}
	return groupKey
}
</script>
